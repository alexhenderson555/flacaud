...
| **Frontend** | **8.5** | apiClient everywhere on main flows; manualChunks; dead code removed. Residual: 1.5MB index chunk (three in landing). |
...
| **Performance** | **8.0** | Lazy routes, virtual lists, vendor chunks. Residual: WebGL landing weight. |
...
**Weighted overall: ~8.4 / 10** — practical maximum without rewriting scope (Tidal dependency, yt-dlp, single-region deploy).
...
1. Lazy-load `@react-three` only on Landing canvas mount
